...
public abstract class RangedWeapon : Equippable
{
    ...
    [Range(0f, 7f)][SerializeField] private float _recoilAmountY;
    [Range(0f, 3f)][SerializeField] private float _recoilAmountX;
    ...
    public virtual bool Shoot(float damage, float range)
    {
        if (CanFire())
        {
            CalculateRecoil();
